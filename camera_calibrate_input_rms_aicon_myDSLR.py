...
class StereoCalibration(object):
    ...
    def calc_rms_about_stereo(self, cal_path, cal_loadjson, cal_loadpoint=None, cal_loadimg=None):
        ...
        if(cal_loadimg != None):
            ...
            for i, fname in enumerate(images_right):
                ...
                gray_l = cv2.blur(gray_l,(3,3))
                gray_r = cv2.blur(gray_r,(3,3))
                gray_l = cv2.adaptiveThreshold(gray_l, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 59, 5)
                gray_r = cv2.adaptiveThreshold(gray_r, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C , cv2.THRESH_BINARY , 59, 5)
                # gray_r = cv2.erode(gray_r,(5,5))
                # ret, gray_l = cv2.threshold(gray_l, 20, 255, cv2.THRESH_BINARY)
                # ret, gray_r = cv2.threshold(gray_r, 24, 255, cv2.THRESH_BINARY)
                # ret, gray_r = cv2.threshold(gray_r, 40, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
                # cv2.imshow('Lst image  _ ' + images_left[i], gray_l)
                # cv2.waitKey(500)
                cv2.imshow('Rst image  _ ' + images_right[i], gray_r)
                cv2.waitKey(0)
    ...
    def read_images_with_mono_stereo(self, cal_path, opt1=None, opt2=None):
        ...
        for i, fname in enumerate(images_right):
            ...
            # gray_l = cv2.adaptiveThreshold(gray_l, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 101, -5)
            # gray_r = cv2.adaptiveThreshold(gray_r, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 101, -5)
            # gray_l = cv2.adaptiveThreshold(gray_l, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 2201, -1)
            # gray_r = cv2.adaptiveThreshold(gray_r, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 2201, -1)
            # gray_l = cv2.adaptiveThreshold(gray_l, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 59, -5)
            # gray_r = cv2.adaptiveThreshold(gray_r, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 59, -5)
            ret, gray_l = cv2.threshold(gray_l, 20, 255, cv2.THRESH_BINARY)
            ret, gray_r = cv2.threshold(gray_r, 20, 255, cv2.THRESH_BINARY)

            # calibration_aicon for 213023000030
            # ret, gray_l = cv2.threshold(gray_l, 25, 255, cv2.THRESH_BINARY)
            # ret, gray_r = cv2.threshold(gray_r, 25, 255, cv2.THRESH_BINARY)
            
            # gray_rr = cv2.resize(gray_r,(1280,720))
            # resized_gray_r = cv2.resize(gray_r, None, fx=0.1, fy=0.1, interpolation=cv2.INTER_LINEAR)
            # cv2.imshow(str(i + 1) + 'st image  _ ' + images_right[i], resized_gray_r)
            # cv2.waitKey(0)
            ...
            if (select_detect_pattern == 1):
                ...
                # flags2 = cv2.CALIB_CB_SYMMETRIC_GRID + cv2.CALIB_CB_CLUSTERING)
                ret_l, corners_l = cv2.findCirclesGrid(gray_l, (marker_point_x, marker_point_y),
            ...
            if ret_l is True and ret_r is True:
                ...
                if (enable_debug_detect_pattern_from_image == 1):
                    resized_img_l = cv2.resize(img_l, None, fx=0.1, fy=0.1, interpolation=cv2.INTER_LINEAR)
                    cv2.imshow(str(i + 1) + 'st image  _ ' + images_left[i], resized_img_l)
                    # cv2.imshow(str(i + 1) + 'st image  _ ' + images_left[i], img_l)
                    # cv2.waitKey(500)
                ...
                if (enable_debug_detect_pattern_from_image == 1):
                    resized_img_r = cv2.resize(img_r, None, fx=0.1, fy=0.1, interpolation=cv2.INTER_LINEAR)
                    cv2.imshow(str(i + 1) + 'st image  _ ' + images_right[i], resized_img_r)
                    cv2.waitKey(0)
                    # cv2.imshow(str(i + 1) + 'st image  _ ' + images_right[i], img_r)
                    # cv2.waitKey(0)
