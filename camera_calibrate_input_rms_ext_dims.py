...
class StereoCalibration(object):
    ...
    def stereo_rectify_with_points(self, img_shape, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, tobjrefpoint, timgpoints_l,timgpoints_r):
        ...
        for i in range(len(tobjrefpoint)):
            ...
            # print('tret', tret)
            # print('tret/tret[3]', (tret/tret[3]).T)
            tdst = cv2.convertPointsFromHomogeneous(tret.T)
            print(tdst.shape)
            t_list_referobj.append(tdst.reshape(-1,3))
    ...
    def depth_using_stereo_param(self, left, right):
		...
		#print('disparity2', disparity, disparity.shape)	
    ...
    def depth_using_stereo(self, img_shape, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, filename_l, filename_r):
        ...
        fx = 1476.77626  # guess for focal length
        Q = np.float32([[1, 0, 0, -0.5 * image_width],
                        [0, -1, 0, 0.5 * image_height],  # turn points 180 deg around x-axis,
        ...
        print(self.Q)
        temp_disparity = disparity.astype(np.float32) / 16.0
        points = cv2.reprojectImageTo3D(temp_disparity, self.Q)
        colors = cv2.cvtColor(img_l, cv2.COLOR_BGR2RGB)
        mask = temp_disparity > 0
        # mask = temp_disparity > temp_disparity.min()
...
#lip        #
    ...
    def calc_rms_stereo2(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T):
        ...
        for i in range(len(obj_points)):
            ...
            # print(A1)
            print('rvec_l tvec_l\n',rvec_l,'\n', tvec_l,'\n')
            print('rvec_l(euler degree)\n',rotationMatrixToEulerAngles(cv2.Rodrigues(np.array(rvec_l))[0])*radianToDegree)
            print('rvec_l(3x3)\n', cv2.Rodrigues(np.array(rvec_l))[0])

            test222 = np.asmatrix(
                #mane king eye
                # [[0.140046196, -0.160750791, 1.215468985, 1 ],
                #  [0.099494186, -0.163309407, 1.200181056, 1 ],
                #  [0.076998926, -0.161599061, 1.199383559, 1 ],
                #  [0.037224325, -0.15600722, 1.213921774, 1 ],
                #  [0.091687215, -0.099701034, 1.180805515, 1 ]]

                #in-lab gaze pos
                [[0.585291464, -0.556549243, -0.200630779, 1],
                 [0.436286199, -0.461982715, -0.20049086, 1],
                 [0.585553698, -0.461906314, -0.200452834, 1],
                 [0.585648716, -0.35917339, -0.200213929, 1],
                 [0.435985314, -0.359093735, -0.200295561, 1],
                 [0.289095887, -0.462116155, -0.200423243, 1],
                 [0.435950387, -0.556125048, -0.200681753, 1],
                 [0.289263687, -0.556396688, -0.200618663, 1],
                 [0.137355007, -0.556400028, -0.200127201, 1],
                 [0.28920385, -0.358922362, -0.200244989, 1],
                 [0.137378263, -0.461921184, -0.200153736, 1],
                 [0.137272491, -0.359033145, -0.199959213, 1],
                 [-0.014523372, -0.462017643, -0.199734116, 1],
                 [-0.0145147, -0.556429129, -0.199690718, 1],
                 [-0.178729686, -0.556369941, -0.19902564, 1],
                 [-0.014707597, -0.358974934, -0.199631719, 1],
                 [-0.178440188, -0.461745353, -0.199207095, 1],
                 [-0.33299202, -0.556225864, -0.198261082, 1],
                 [-0.333172096, -0.461670868, -0.198334073, 1],
                 [-0.332712978, -0.35901615, -0.198374524, 1],
                 [0.585680513, -0.256449323, -0.199750636, 1],
                 [0.436163965, -0.256183553, -0.199907353, 1],
                 [0.585567095, -0.153378366, -0.199095143, 1],
                 [0.28923698, -0.25622733, -0.19977866, 1],
                 [0.436214183, -0.153641049, -0.199185484, 1],
                 [0.585598963, -0.050567454, -0.198344741, 1],
                 [0.436286847, -0.050437906, -0.198391954, 1],
                 [0.289364272, -0.153257265, -0.199108724, 1],
                 [0.288966197, -0.050455636, -0.198413852, 1],
                 [0.137296046, -0.256295854, -0.199589174, 1],
                 [0.137272915, -0.153151667, -0.198999284, 1],
                 [-0.014409785, -0.255887045, -0.19937436, 1],
                 [0.137242556, -0.050174387, -0.198377585, 1],
                 [-0.014486906, -0.153295787, -0.198942, 1],
                 [-0.178693683, -0.153149538, -0.198629825, 1],
                 [-0.014417599, -0.050445498, -0.198401669, 1],
                 [-0.332777779, -0.050206274, -0.197508088, 1],
                 [-0.332827715, -0.15325101, -0.198021902, 1],
                 [-0.178775325, -0.256076654, -0.199066454, 1],
                 [-0.332948354, -0.256018399, -0.198301509, 1],
                 [-0.478929228, -0.555227758, -0.096286337, 1],
                 [-0.478952686, -0.460676069, -0.096120049, 1],
                 [-0.668781188, -0.254655667, 0.07895042, 1],
                 [-0.479099029, -0.357277082, -0.095967601, 1],
                 [-0.47909151, -0.254498367, -0.095752044, 1],
                 [-0.478809039, -0.151558117, -0.095631152, 1],
                 [-0.668662069, -0.151393113, 0.079174993, 1],
                 [-0.478366106, -0.048747181, -0.095434944, 1],
                 [-0.668793944, -0.048739758, 0.079643903, 1],
                 [-0.857885072, -0.048490825, 0.253935264, 1],
                 [-1.047289304, -0.048552352, 0.429446665, 1],
                 [-0.858289514, -0.151490748, 0.25402718, 1],
                 [-0.858387784, -0.254475728, 0.253906217, 1],
                 [-1.047417124, -0.151515751, 0.429361925, 1],
                 [-1.047617696, -0.254374833, 0.429569347, 1],
                 [-0.858321363, -0.357051943, 0.253772679, 1],
                 [-0.669208761, -0.357163069, 0.07911977, 1],
                 [-0.669229331, -0.460231827, 0.079321998, 1],
                 [-0.858156411, -0.460059345, 0.253818821, 1],
                 [-1.047134217, -0.554409037, 0.429820971, 1],
                 [0.535573163, 0.062612502, 0.36177359, 1],
                 [0.535734406, 0.157051081, 0.362542033, 1],
                 [0.232081581, 0.260111976, 0.36209314, 1],
                 [0.535566335, 0.26003004, 0.363338107, 1],
                 [0.535650198, 0.362637796, 0.364197931, 1],
                 [0.535717946, 0.465753884, 0.365221407, 1],
                 [0.232129422, 0.362702464, 0.362939994, 1],
                 [0.535962117, 0.568247124, 0.366509669, 1],
                 [0.231874732, 0.465555214, 0.363624703, 1],
                 [-0.071447364, 0.46620185, 0.362598198, 1],
                 [-0.375118447, 0.569043229, 0.36188071, 1],
                 [-0.071486016, 0.362946789, 0.362176709, 1],
                 [-0.07186416, 0.260412758, 0.361713154, 1],
                 [-0.375607385, 0.466068107, 0.361821311, 1],
                 [-0.375502918, 0.363429183, 0.361564744, 1],
                 [-0.072076622, 0.157416218, 0.361364051, 1],
                 [0.231955765, 0.157260929, 0.361383235, 1],
                 [0.231832484, 0.062526323, 0.360953916, 1],
                 [-0.072226113, 0.062829057, 0.361128107, 1],
                 [-0.376066529, 0.063298056, 0.361911365, 1],
                 [-0.502693307, 0.033622641, 0.496186067, 1],
                 [-0.502891525, 0.127828908, 0.496153588, 1],
                 [-0.7118902, 0.333680954, 0.648618745, 1],
                 [-0.503116114, 0.231159488, 0.496261921, 1],
                 [-0.503209354, 0.334291226, 0.49662085, 1],
                 [-0.502819661, 0.436750031, 0.497044697, 1],
                 [-0.711972185, 0.436491349, 0.64906199, 1],
                 [-0.502762009, 0.539535335, 0.497933838, 1],
                 [-0.711712221, 0.5394162, 0.648928187, 1],
                 [-0.920361193, 0.539081303, 0.800989973, 1],
                 [-1.129131389, 0.53897726, 0.952447309, 1],
                 [-0.920473404, 0.436129665, 0.800779606, 1],
                 [-0.920829312, 0.333472998, 0.800588737, 1],
                 [-1.129258672, 0.435957985, 0.95211349, 1],
                 [-1.129302308, 0.333345002, 0.951951994, 1],
                 [-0.920787348, 0.230584232, 0.800422517, 1],
                 [-0.711832611, 0.230692888, 0.648391916, 1],
                 [-0.711681703, 0.127693834, 0.648333646, 1],
                 [-0.920386444, 0.127839151, 0.800411969, 1],
                 [-1.127857342, 0.032973232, 0.952324384, 1],
                 [0.595367508, -0.499463897, 0.388078892, 1],
                 [0.595697796, -0.19584547, 0.387944279, 1],
                 [0.810873078, -0.499471923, 0.521858091, 1],
                 [0.739388576, -0.195908797, 0.476374685, 1],
                 [0.595526337, 0.107767306, 0.387449105, 1],
                 [0.811100006, 0.107884207, 0.520759966, 1],
                 [0.88232638, -0.196066433, 0.565651231, 1],
                 [1.024898961, 0.107851031, 0.655590854, 1],
                 [1.024902073, -0.195873212, 0.655824422, 1],
                 [1.024595017, -0.499407456, 0.656574881, 1],
                 [-1.182029491, -0.220621606, 0.573282278, 1],
                 [-1.177554873, -0.051358472, 0.573353786, 1],
                 [-1.188836253, -0.220577168, 0.723662955, 1],
                 [-1.185393933, -0.053208266, 0.723227549, 1],
                 [-1.194457209, -0.222621596, 0.873170957, 1],
                 [-1.192068855, -0.052732871, 0.872709455, 1],
                 [-1.198950955, -0.220241779, 1.022486293, 1],
                 [-1.197238002, -0.051269829, 1.022416804, 1],
                 [-1.192063466, 0.124655767, 1.022082245, 1],
                 [-1.203450291, -0.220432455, 1.171937396, 1],
                 [-1.201705499, -0.050692095, 1.172043862, 1],
                 [-1.197101528, 0.124311848, 1.171694977, 1],
                 [-1.207537218, -0.218947486, 1.323693452, 1],
                 [-1.205517065, -0.051664131, 1.322011522, 1],
                 [-1.201128937, 0.124582854, 1.322628269, 1],
                 # in-lab gaze pos -add 4 point
                 [0.453151117, -0.008634924, 0.002481871, 1],
                 [0.224752264, -0.007956347, 0.002610259, 1],
                 [-0.075490984, -0.008395195, 0.002535888, 1],
                 [-0.312886718, -0.00901204, 0.002433717, 1],
                 # in-lab ABS3_P1, P2, P3
                 [0.033803,	    -0.018475,    0.000000,   1],
                 [0.128498,    0.000000,    0.000000,   1],
                 [0.000000,    0.000000,    0.000000,   1],

                 #  # in-veh gaze pos
                # [[0.050755324, 0.210732029, -0.024095562, 1],
                #  [-0.015066848, 0.050683114, -0.055849608, 1],
                #  [0.090625273, 0.057326976, -0.001451284, 1],
                #  [-0.024770074, 0.353776784, -0.089090022, 1],
                #  [0.091532051, 0.36288989, -0.019820461, 1],
                #  [0.069836331, -0.175716951, -6.02418E-05, 1],
                #  [-0.012432803, -0.34538744, -0.000338975, 1],
                #  [0.114056728, -0.343586216, -0.000314749, 1],
                #  [-0.023637553, -0.001083105, -0.004373931, 1],
                #  [-0.62365571, -0.476127989, -0.088289529, 1],
                #  [-0.602502048, -0.433563132, 0.352861276, 1],
                #  [-0.600352378, -0.045209447, 0.338585027, 1],
                #  [-0.262347017, 0.446231856, -0.001433396, 1],
                #  [-0.125012313, 0.463744628, -0.071504156, 1],
                #  [-0.260812999, 0.874826631, -0.010908553, 1],
                #  [-0.116208225, 0.865302643, -0.07508353, 1],
                #  [-0.634314821, 0.4091303, -0.120333494, 1],
                #  [-0.649008806, 0.846153734, -0.153802281, 1],
                #  [-0.626047712, 0.855632612, 0.301412642, 1],
                #  [0.365564152, -0.147589442, -0.461219335, 1],
                #  [0.359211919, -0.326364601, -0.41548469, 1],
                #  [0.364387755, -0.310306521, -0.523123517, 1],
                #  [0.364085521, 0.028567939, -0.363439379, 1],
                #  [0.374081971, 0.020094944, -0.512601757, 1],
                #  [-0.247941999, 0.184298858, -0.333954072, 1],
                #  [-0.277367008, 0.080583219, -0.421364753, 1],
                #  [-0.300688263, 0.076742211, -0.21676768, 1],
                #  [-0.277997255, 0.270497948, -0.431302821, 1],
                #  [-0.303758691, 0.294522569, -0.226224589, 1],
                #  [0.202468188, -0.146234017, 0.17619778, 1],
                #  [0.402349559, -0.196291185, -0.264885288, 1],
                #  [0.13737064, -0.451861035, 0.238177459, 1],
                #  [0.286023595, -0.428485637, -0.055320371, 1],
                #  [0.381639739, -0.384876472, -0.274541425, 1],
                #  [0.204736313, 0.266946062, 0.174322006, 1],
                #  [0.398646463, 0.117342461, -0.238866604, 1],
                #  [0.184843854, 0.549780197, 0.163625318, 1],
                #  [0.389625017, 0.565580404, -0.288117763, 1],
                #  [0.283144906, 0.513529203, -0.019969207, 1],
                #  [0.169073052, 0.88012659, 0.077419479, 1],
                #  [0.289983374, 0.820964276, -0.155416487, 1],
                #  [0.369599437, 0.734857067, -0.33841693, 1],
                #  [-0.009251268, 0.073230134, -0.046985993, 1],
                #  [0.075890139, 0.077586797, -0.002233275, 1],
                #  [-0.01081327, 0.351594832, -0.065563607, 1],
                #  [0.072898884, 0.354245586, -0.021655794, 1],
                #  [0.161663881, -0.602560323, -0.508502985, 1],
                #  [0.355107558, -0.500158894, -0.475189404, 1],
                #  [0.132567173, -0.758805754, -0.029717416, 1],
                #  [0.082657473, -0.825086141, -0.044627371, 1],
                #  [0.162009106, -0.821257212, -0.042667481, 1],
                #  [0.077133015, -0.712939708, -0.017073917, 1],
                #  [0.184039858, -0.680809703, -0.005286281, 1],
                #  [0.283473675, 0.513385807, -0.020037262, 1],
                #  [0.093891848, 1.16359839, -0.096651589, 1],
                #  [0.047652717, 1.110565375, -0.063294111, 1],
                #  [0.154066317, 1.084218321, -0.047610906, 1],
                #  [0.047698977, 1.221499084, -0.125592248, 1],
                #  [0.116547926, 1.225453793, -0.129148872, 1],
                #  [0.374205705, 0.022736012, -0.50562199, 1],
                #  [-0.303253813, 0.293814157, -0.220756471, 1],
                #  [-0.276461748, 0.083854997, -0.428585022, 1],
                #  [-0.300267949, 0.083284433, -0.217207767, 1],
                #  [-0.279243419, 0.272063616, -0.438169684, 1],
                #  [0.3742165, -0.378706093, -0.247723453, 1],
                #
                #  #vehicle ABS3_P1, P2, P3
                #  [0.099745,   -0.066386,    0.000000,   1],
                #  [0.095506,    0.000000,    0.000000,   1],
                #  [0.000000,    0.000000,    0.000000,   1],
            ...
            t_matrix = np.eye(4)
            t_matrix[0:3, 0:3] = cv2.Rodrigues(np.array(rvec_l))[0]
            t_matrix[0:3, 3] = tvec_l.T
            print('t_matrix', t_matrix)
            result222 = np.asmatrix(t_matrix * test222.T)
            print("result222", result222.T)
            # euler = rotationMatrixToEulerAngles(t_matrix_inv[0:3, 0:3]) * radianToDegree
    ...
    def calc_rms_stereo4(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T, tLog=True):
        tot_error = 0
        total_points = 0
        p_reproj_left = []
        p_reproj_right = []
        ...
        # print('total_ext_r', ext_r,'total_ext_t', ext_t)
        # print('avg_ext_r/N', ext_r/len(obj_points),'avg_ext_t/N', ext_t/len(obj_points) )
    ...
    def check_boundary_until_rp(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T):
        ...
        offset_t = 0.005
        tfact = []
        ta = np.linspace(R[0][0] - offset_r, R[0][0] + offset_r, num=10)
        tb = np.linspace(R[0][1] - offset_r, R[0][1] + offset_r, num=10)
        tc = np.linspace(R[0][2] - offset_r, R[0][2] + offset_r, num=10)
        td = np.linspace(T[0] - offset_t, T[0] + offset_t, num=11)
        te = np.linspace(T[1] - offset_t, T[1] + offset_t, num=11)
        tf = np.linspace(T[2] - offset_t, T[2] + offset_t, num=11)
        ...
        for tdata in tfact:
            ...
            print("\n\n")
            RR = R.copy()
            for tRBound in range(int(len(tdata)/2),len(tdata), 1):
                print(tRBound)
                print(tdata[tRBound])
                RR[0][0] = tdata[tRBound]
                stero_rms, re_left, re_right = self.calc_rms_stereo4(obj_points, imgpoints_l, imgpoints_r,
                                                                     A1, D1, A2, D2, RR, T, tLog=False)
                print("rmse ",stero_rms)
            for tLBound in range(int(len(tdata)/2), -1, -1):
                print(tLBound)
                print(tdata[tLBound])
                RR[0][0] = tdata[tLBound]
                stero_rms, re_left, re_right = self.calc_rms_stereo4(obj_points, imgpoints_l, imgpoints_r,
                                                                     A1, D1, A2, D2, RR, T, tLog=False)
                print("rmse ",stero_rms)
        print("test end")
        ...
        return stero_rms, re_left, re_right
