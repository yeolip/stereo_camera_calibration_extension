...
def load_value_from_json(filename):
    ...
    if (s_ttrans[0] == 0 and s_ttrans[1] == 0 and s_ttrans[2] == 0 and s_trot[0] == 0 and s_trot[1] == 0 and s_trot[2] == 0):
        ...
        if (m_fx < 0 and m_fy < 0):
            ...
            roty = - roty
    #################################################################3
    print('|master    [ %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f]'%(m_fx, m_fy, m_cx, m_cy, m_k1, m_k2, m_k3, m_k4))
    print('|slave     [ %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f]'%(s_fx, s_fy, s_cx, s_cy, s_k1, s_k2, s_k3, s_k4))
    ...
    # return value is plus focal length, and position of rot and trans from left to right
    return m_fx, m_fy, m_cx, m_cy, m_k1, m_k2, m_k3, m_k4, s_fx, s_fy, s_cx, s_cy, s_k1, s_k2, s_k3, s_k4, tranx, trany, tranz, \
...
def modify_value_from_json(path, filename, M1, d1, M2, d2, R, T, imgsize, ret_rp, E=None, F=None):
    ...
    fjs["master"]["lens_params"]['k1'] = td1[0]
    fjs["master"]["lens_params"]['k2'] = td1[1]
    fjs["master"]["lens_params"]['k3'] = td1[2]
    fjs["master"]["lens_params"]['k4'] = td1[3]
    fjs["slave"]["lens_params"]['focal_len'] = tM2[0][0], tM2[1][1]
    fjs["slave"]["lens_params"]['principal_point'] = tM2[0][2], tM2[1][2]
    fjs["slave"]["lens_params"]['k1'] = td2[0]
    fjs["slave"]["lens_params"]['k2'] = td2[1]
    fjs["slave"]["lens_params"]['k3'] = td2[2]
    fjs["slave"]["lens_params"]['k4'] = td2[3]
    print("*" * 50)
...
def least_squares_stereo_rmse2(x, tobj_point, timgpoint_l, timgpoint_r, A1, D1, A2, D2, R, T):
    ...
    # compute reprojection error for cam2
    rp_r, _ = cv2.fisheye.projectPoints(tobj_point, rvec_r2, tvec_r2, A2, D2)
    # print( 'rp_r' , rp_r )
    # tot_error += np.square(rp_r - t_imgpoints_r).sum()
    tot_error += np.sum(np.square(np.float64(rp_r - timgpoint_r)))
    total_points += len(tobj_point)
...
def least_squares_stereo_rmse3(x, R, T):
    ...
    rvec_r3, tvec_r3 = cv2.fisheye.composeRT(rvec_r2, tvec_r2, -rvec_r, -tvec_r)[:2]
...
class StereoCalibration(object):
    ...
    def loop_moving_of_rot_and_trans(self, cal_path, rot, tran):
        ...
        while (1):
            ...
            if(enable_debug_loop_moving_of_rot_and_trans == 1):
                reprojected_points, _ = cv2.fisheye.projectPoints(self.objpoints[filenum], uR33, uT, self.M1, self.d1)
    ...
    def calc_rms_about_stereo(self, cal_path, cal_loadjson, cal_loadpoint=None, cal_loadimg=None):
        ...
        if(cal_loadpoint != None):
            ...
            for i, fname in enumerate(loadpoint):
                ...
                self.objpoints.append(np.asarray(ref_point))
                # self.objpoints.append(self.objp_center)
                self.imgpoints_l.append(np.asarray(img_point_l))
                self.imgpoints_r.append(np.asarray(img_point_r))
            self.imgpoints_l = np.expand_dims(np.asarray(self.imgpoints_l), -2)
            self.imgpoints_r = np.expand_dims(np.asarray(self.imgpoints_r), -2)

        self.objpoints = np.expand_dims(np.asarray(self.objpoints), -2)

        m_fx, m_fy, m_cx, m_cy, m_k1, m_k2, m_k3, m_k4, s_fx, s_fy, s_cx, s_cy, s_k1, s_k2, s_k3, s_k4, tranx, trany, tranz, rotx, roty, rotz, calib_res \
        ...
        print('ret [ %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f]' % (m_fx, m_fy, m_cx, m_cy, m_k1, m_k2, m_k3, m_k4))
        print('ret [ %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f, %.8f]' % (s_fx, s_fy, s_cx, s_cy, s_k1, s_k2, s_k3, s_k4))
    ...
    def read_points_with_mono_stereo(self, cal_path, cal_loadjson=None, cal_loadpoint=None, opt1=None, opt2=None):
        ...
        for i, fname in enumerate(loadpoint):
            ...
            #ref_point2 = np.expand_dims(np.asarray(ref_point), -2)
        ...
        print('ret [ %.6f, %.6f, %.6f, %.6f/ %.8f, %.8f, %.8f, %.8f]' % (m_fx, m_fy, m_cx, m_cy, m_k1, m_k2, m_k3, m_k4))
        print('ret [ %.6f, %.6f, %.6f, %.6f/ %.8f, %.8f, %.8f, %.8f]' % (s_fx, s_fy, s_cx, s_cy, s_k1, s_k2, s_k3, s_k4))
        print('ret | trans [ %.8f, %.8f, %.8f]' % (tranx, trany, tranz), 'rot_radian[ %.8f, %.8f, %.8f]' % (rotx, roty, rotz))
    ...
    def read_points_with_stereo(self, cal_path, cal_loadjson=None, cal_loadpoint=None, opt2=None):
        ...
        dist_coef_l = np.zeros((4,1), np.float32)
        dist_coef_l[0] = m_k1
        dist_coef_l[1] = m_k2
        dist_coef_l[2] = m_k3
        dist_coef_l[3] = m_k4
        ...
        dist_coef_r = np.zeros((4, 1), np.float32)
        dist_coef_r[0] = s_k1
        dist_coef_r[1] = s_k2
        dist_coef_r[2] = s_k3
        dist_coef_r[3] = s_k4
...
#        print('ret [ %.6f, %.6f, %.6f, %.6f/ %.8f, %.8f, %.8f, %.8f]' % (camera_matrix_r[0][0], camera_matrix_r[1][1], camera_matrix_r[0][2], camera_matrix_r[1][2], dist_coef_r[0][0], dist_coef_r[0][1], dist_coef_r[0][2], dist_coef_r[0][3]))
        ...
        rt, self.M1, self.d1, self.r1, self.t1 = cv2.fisheye.calibrate(self.objpoints, self.imgpoints_l, img_shape,
                                                                     camera_matrix_l,
                                                                     dist_coef_l, flags=flags)
        # objpoints2 = np.expand_dims(np.asarray(self.objpoints), -2)
        rt2, self.M2, self.d2, self.r2, self.t2 = cv2.fisheye.calibrate(self.objpoints, self.imgpoints_r, img_shape,
                                                                      camera_matrix_r,
    ...
    def stereo_rectify(self, img_shape, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, filename_l, filename_r):
        ...
        mapL1, mapL2 = cv2.fisheye.initUndistortRectifyMap(camera_matrix_l, dist_coeffs_l, RL, PL, img_shape, cv2.fisheye.CV_32FC1);
        mapR1, mapR2 = cv2.fisheye.initUndistortRectifyMap(camera_matrix_r, dist_coeffs_r, RR, PR, img_shape, cv2.fisheye.CV_32FC1);
        ...
        undistorted_rectifiedL = cv2.fisheye.remap(gray_l, mapL1, mapL2, cv2.fisheye.INTER_LINEAR);
        undistorted_rectifiedR = cv2.fisheye.remap(gray_r, mapR1, mapR2, cv2.fisheye.INTER_LINEAR);
        ...
        dst = cv2.fisheye.undistortPoints(self.imgpoints_l[0], camera_matrix_l, dist_coeffs_l, R=RL, P=PL)
        dst2 = cv2.fisheye.undistortPoints(self.imgpoints_r[0], camera_matrix_r, dist_coeffs_r, R=RR, P=PR)
        # cv::triangulatePoints(P1, P2, imgWutBok, imgWutTyl, point4D);
        tret = cv2.fisheye.triangulatePoints(PL,PR,np.array(t_lpoint).reshape(-1, 1, 2), np.array(t_rpoint).reshape(-1, 1, 2))
        print('tret', tret)
        ...
        rp_r, _ = cv2.fisheye.projectPoints(self.objpoints[0], tR2, tT2, tmat2, tmat_d)
        # print('rp_l', rp_l)
        # print('rp_r', rp_r)
        ...
        # lines1 = cv2.fisheye.computeCorrespondEpilines(pts2, 2, self.F)
        ...
        # lines2 = cv2.fisheye.computeCorrespondEpilines(pts1, 1, self.F)
    ...
    def stereo_rectify_with_points(self, img_shape, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, tobjrefpoint, timgpoints_l,timgpoints_r):
        ...
        for i in range(len(tobjrefpoint)):
            ...
            un_dst_l = cv2.fisheye.undistortPoints(temp_lpoints, camera_matrix_l, dist_coeffs_l, R=RL, P=PL)
            un_dst_r = cv2.fisheye.undistortPoints(temp_rpoints, camera_matrix_r, dist_coeffs_r, R=RR, P=PR)
            ...
            un_dst_l = [un_dst_l]
            un_dst_r = [un_dst_r]
            tret = cv2.fisheye.triangulatePoints(PL, PR, np.array(un_dst_l).reshape(-1, 1, 2), np.array(un_dst_r).reshape(-1, 1, 2))
            # print('tret', tret)
            # print('tret/tret[3]', (tret/tret[3]).T)
            tdst = cv2.fisheye.convertPointsFromHomogeneous(tret.T)
            print(tdst.shape)
        ...
        for i in range(len(tobjrefpoint)):
            # _, rvec_l, tvec_l, _ = cv2.fisheye.solvePnPRansac(tobjrefpoint[i], t_list_left[i], camera_matrix_l, dist_coeffs_l)
            # _, rvec_r, tvec_r, _ = cv2.fisheye.solvePnPRansac(tobjrefpoint[i], t_list_right[i], camera_matrix_r, dist_coeffs_r)
            _, rvec_l, tvec_l, _ = cv2.fisheye.solvePnPRansac(tobjrefpoint[i], t_list_left[i], tmat, tmat_d)
            ...
            rp_r_prime, _ = cv2.fisheye.projectPoints(tobjrefpoint[i], tR2, tT2, tmat2, tmat_d)
            # print('\nrp_l\n', rp_l)
            # print('\nrp_r\n', rp_r)
            # print('\nrp_r_prime\n', rp_r_prime)
    ...
    def depth_using_stereo_param(self, left, right):
        ...
        sbm = cv2.fisheye.StereoBM_create(numDisparities=disparities, blockSize=block)
        ...
        # plt.imshow(depth)
        depth = cv2.fisheye.equalizeHist(depth)
        colorized_depth = np.zeros((left.shape[0], left.shape[1], 3), dtype="uint8")
        temp = cv2.fisheye.applyColorMap(depth, cv2.fisheye.COLORMAP_JET)
    ...
    def depth_using_stereo(self, img_shape, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, filename_l, filename_r):
        ...
        # FROM manual: stereoProcessor = cv2.fisheye.StereoSGBM(numDisparities=128, SADWindowSize=21);
        # From help(cv2.fisheye.: StereoBM_create(...)
        ...
        max_disparity = 128;
        stereoProcessor = cv2.fisheye.StereoSGBM_create(0, max_disparity, 21);
        ...
        undistorted_rectifiedL = cv2.fisheye.remap(gray_l, self.mapL1, self.mapL2, cv2.fisheye.INTER_LINEAR);
        undistorted_rectifiedR = cv2.fisheye.remap(gray_r, self.mapR1, self.mapR2, cv2.fisheye.INTER_LINEAR);
        ...
        cv2.fisheye.filterSpeckles(disparity, 0, 4000, 128);
        ...
        points = cv2.fisheye.reprojectImageTo3D(temp_disparity, self.Q)
...
#lip        #
    ...
    def draw_xyz_axis(self, img, corners, imgpts):
        ...
        img = cv2.fisheye.line(img, corner, tuple(imgpts[0].ravel()), (255, 0, 0), 5)
        img = cv2.fisheye.line(img, corner, tuple(imgpts[1].ravel()), (0, 255, 0), 5)
        img = cv2.fisheye.line(img, corner, tuple(imgpts[2].ravel()), (0, 0, 255), 5)
        return img
            ...
            if (enable_debug_pose_estimation_display == 1 or enable_debug_pose_estimation_display == 2 ):
                ...
                if ret == True:
                    if (select_detect_pattern == 1):
                        ...
                        ret, rvecs, tvecs = cv2.fisheye.solvePnP(self.objp, corners_l, mtx, dist)
                        imgpts, jac = cv2.fisheye.projectPoints(self.axis, rvecs, tvecs, mtx, dist)
            ...
            if (enable_debug_pose_estimation_display == 1 or enable_debug_pose_estimation_display == 3 ):
                ...
                if ret == True:
                    if (select_detect_pattern == 1):
                        ...
                        ret, rvecs, tvecs = cv2.fisheye.solvePnP(self.objp, corners_r, mtx, dist)
                        imgpts, jac = cv2.fisheye.projectPoints(self.axis, rvecs, tvecs, mtx, dist)
    ...
    def reprojection_error(self, obj_points, img_points, rvecs, tvecs, camera_matrix, dist_coeffs):
        ...
        # ret, temp_rvecs, temp_tvecs = cv2.fisheye.solvePnP(obj_points[0], img_points[0], camera_matrix, dist_coeffs, flags=cv2.fisheye.SOLVEPNP_ITERATIVE  )
        # print('temp r t', temp_rvecs, temp_tvecs)
        # ret, temp_rvecs, temp_tvecs = cv2.fisheye.solvePnP(obj_points[0], img_points[0], camera_matrix, dist_coeffs, flags=cv2.fisheye.SOLVEPNP_EPNP  )
        # print('temp r t', temp_rvecs, temp_tvecs)
        # ret, temp_rvecs, temp_tvecs = cv2.fisheye.solvePnP(obj_points[0], img_points[0], camera_matrix, dist_coeffs, flags=cv2.fisheye.SOLVEPNP_UPNP  )
        # print('temp r t', temp_rvecs, temp_tvecs)
        ...
        ret_error = 0
        for i in range(len(obj_points)):
            reprojected_points, _ = cv2.fisheye.projectPoints(obj_points[i], rvecs[i], tvecs[i], camera_matrix, dist_coeffs)
            reprojected_points = reprojected_points.reshape(-1, 2)
    ...
    def reprojection_error2(self, obj_points, img_points, camera_matrix, dist_coeffs):
        ...
        total_points = 0
        for i in range(len(obj_points)):
            undistorted = cv2.fisheye.undistortPoints(img_points[i], camera_matrix, dist_coeffs)
            Keye = np.eye(3)
            Dzero = np.zeros((1, 5))
            ret, temp_rvecs, temp_tvecs = cv2.solvePnP(obj_points[i], undistorted, Keye, Dzero, flags=cv2.SOLVEPNP_ITERATIVE)
            ...
            reprojected_points, _ = cv2.fisheye.projectPoints(obj_points[i], temp_rvecs, temp_tvecs, camera_matrix, dist_coeffs)
            reprojected_points = reprojected_points.reshape(-1, 2)
            temp_points = img_points_reshape[i].reshape(-1, 2)
    ...
    def calc_rms_stereo(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T):
        ...
        for i in range(len(obj_points)):
            ...
            #cv2.fisheye.FindExtrinsicCameraParams2(obj_points[i], imgpoints_l[i], A1, D1, temp_rvecs, temp_tvecs)
            _, rvec_l, tvec_l, _ = cv2.fisheye.solvePnPRansac(obj_points[i], imgpoints_l[i], A1, D1 )
    ...
    def calc_rms_stereo2(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T):
        ...
        for i in range(len(obj_points)):
            ...
            #_, rvec_l, tvec_l, _ = cv2.fisheye.solvePnPRansac(obj_points[i], t_imgpoints_l, A1, D1 )
            #_, t_rvec_l, t_tvec_l = cv2.fisheye.solvePnP(obj_points[i], imgpoints_l[i], A1, D1, useExtrinsicGuess = True, flags=cv2.fisheye.SOLVEPNP_ITERATIVE )
            # A1[0][0] = -A1[0][0]
            # A1[1][1] = -A1[1][1]
    ...
    def calc_rms_stereo4(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T, tLog=True):
        ...
        for i in range(len(obj_points)):
            ...
            _, rvec_l, tvec_l = cv2.fisheye.solvePnP(obj_points[i], t_imgpoints_l, A1, D1, flags=cv2.fisheye.SOLVEPNP_ITERATIVE)
            _, rvec_r, tvec_r = cv2.fisheye.solvePnP(obj_points[i], t_imgpoints_r, A2, D2, flags=cv2.fisheye.SOLVEPNP_ITERATIVE)
            ...
            rp_l, _ = cv2.fisheye.projectPoints(obj_points[i], rvec_l.reshape(3,1), tvec_l.reshape(3,1), A1, D1)
            # print( 'rp_l' , rp_l )
            # tot_error += np.sum(np.square(np.float64(imgpoints_l[i] - rp_l)))
            tot_error += np.sum(np.square(np.float64(rp_l - t_imgpoints_l)))
            total_points += len(obj_points[i])
            ...
            # compute reprojection error for cam2
            rp_r, _ = cv2.fisheye.projectPoints(obj_points[i], rvec_r2, tvec_r2, A2, D2)
            # print( 'rp_r' , rp_r )
            # tot_error += np.square(rp_r - t_imgpoints_r).sum()
            tot_error += np.sum(np.square(np.float64(rp_r - t_imgpoints_r)))
            total_points += len(obj_points[i])
    ...
    def calc_rms_stereo_detail(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T):
        ...
        for i in range(len(obj_points)):
            ...
            # _, rvec_l, tvec_l, _ = cv2.fisheye.solvePnPRansac(obj_points[i], t_imgpoints_l, A1, D1, rvec = res['x'][0:3], tvec = res['x'][3:6], useExtrinsicGuess=True ,flags=cv2.fisheye.SOLVEPNP_ITERATIVE)
            # _, rvec_l, tvec_l, _ = cv2.fisheye.solvePnPRansac(obj_points[i], t_imgpoints_l, A1, D1 ,flags=cv2.fisheye.SOLVEPNP_ITERATIVE, reprojectionError=0.25, iterationsCount=300)

            # if(i==0):
            _, rvec_l, tvec_l = cv2.fisheye.solvePnP(obj_points[i], t_imgpoints_l, A1, D1, flags=cv2.fisheye.SOLVEPNP_ITERATIVE)
            # _, rvec_r, tvec_r = cv2.fisheye.solvePnP(obj_points[i], t_imgpoints_r, A2, D2, flags=cv2.fisheye.SOLVEPNP_ITERATIVE)
           # else:
            #     _, rvec_l, tvec_l = cv2.fisheye.solvePnP(obj_points[i], t_imgpoints_l, A1, D1, flags=cv2.fisheye.SOLVEPNP_ITERATIVE, rvec = rvec_l, tvec=tvec_l, useExtrinsicGuess=True)
            pre_rvec_l = rvec_l
            pre_tvec_l = tvec_l
            # print('\tR_L', rvec_l[0],rvec_l[1],rvec_l[2],'\n\tT_L', tvec_l[0],tvec_l[1],tvec_l[2])
            # flog.write('\tR_L [ %.8f, %.8f, %.8f]\n\tT_L [ %.8f, %.8f, %.8f]\n'%(rvec_l[0],rvec_l[1],rvec_l[2],tvec_l[0],tvec_l[1],tvec_l[2]))
    ...
    def calc_rms_stereo6(self, obj_points, imgpoints_l, imgpoints_r, A1, D1, A2, D2, R, T, tLog=True):
        ...
        for i in range(len(obj_points)):
            ...
            undistorted_l = cv2.fisheye.undistortPoints(t_imgpoints_l, A1, D1)
            undistorted_r = cv2.fisheye.undistortPoints(t_imgpoints_r, A2, D2)
            Keye = np.eye(3)
            Dzero = np.zeros((1, 5))

            # Left optimizer
            _, rvec_l, tvec_l = cv2.solvePnP(obj_points[i], undistorted_l, Keye, Dzero, flags=cv2.SOLVEPNP_ITERATIVE)
            _, rvec_r, tvec_r = cv2.solvePnP(obj_points[i], undistorted_r, Keye, Dzero, flags=cv2.SOLVEPNP_ITERATIVE)
            #_, rvec_l, tvec_l = cv2.fisheye.solvePnP(obj_points[i], t_imgpoints_l, A1, D1, flags=cv2.fisheye.SOLVEPNP_ITERATIVE)
            #_, rvec_r, tvec_r = cv2.fisheye.solvePnP(obj_points[i], t_imgpoints_r, A2, D2, flags=cv2.fisheye.SOLVEPNP_ITERATIVE)
    ...
    def draw_crossline(self, img, x, y, color, thinkness):
        ...
        if(x > width and y > width):
            cv2.fisheye.line(img, (int(x), int(y-width)), (int(x), int(y+width)), color, thinkness)
            cv2.fisheye.line(img, (int(x-width), int(y)), (int(x+width), int(y)), color, thinkness)
        else:
    ...
    def draw_arrow(self, img, start_x, start_y, end_x, end_y, color, thinkness, scale=100):
        ...
        if(start_x > width and start_y > width):
            # cv2.fisheye.line(img, (int(start_x), int(start_y)), (int(end_x) , int(end_y)), color, thinkness)
            # cv2.fisheye.line(img, (int(start_x), int(start_y)), (int(gap_x) , int(gap_y)), color, thinkness)
            cv2.fisheye.arrowedLine(img, (start_x, start_y), (gap_x, gap_y), (0, 0, 255), thickness=thinkness, tipLength=0.1)
        else:
            print("error - invalid point")
        pass
